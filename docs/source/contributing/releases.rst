...
9. Finally we need to update the `conda-forge cset feedstock`_ to publish an
   updated conda package. The main change that will need to be made is updating
   the version and SHA256 hash in the ``recipe/recipe.yaml`` file. The hash can
   be obtained from the `PyPI release page`_ > "Download files" > "View details"
